...
version: v1.12
updated: 2026-08-12
status: aprovado — v1.12, em 2026-08-12
tipo: prd
...
>
> ✅ **Aprovado até a v1.12, em 2026-08-12.** As seis versões escritas neste dia
> passaram juntas: a v1.7 fechou seis desvios entre este documento e o
> protótipo; a v1.8 registrou onze desacordos novos (X1 a X11), achados na
> auditoria cruzada entre PRD, protótipo e app implementado, e criou o portão;
> a v1.9 fechou as duas divergências que dependiam do dono:
>
...
>
> 🔴 **A aprovação não fecha a pendência que sobrou, e ela é a mais cara das
> três:** a leitura da conta do checkpoint (seção 7). Aprovar as versões quer
> dizer que o **texto** está aceito — não que a conta esteja confirmada. Ela
> continua marcada em 🔴 na seção 7, e continua sendo decisão do dono do
> processo: é a conta que decide se o caixa fecha, e se estiver errada muda o
> produto inteiro.
>
...
| H5 | O custo até o piloto cabe no que não se paga: sem publicação em loja, sem API paga de WhatsApp | Premissa R6 do briefing, ainda **não confirmada**. Se algum desses custar, muda o prazo ou o escopo. ⚠️ **Recalibrada em 2026-08-13 pela DEC-007:** a linha dizia "Supabase no plano gratuito", e o Supabase saiu. Agora existe servidor, e servidor não tem plano gratuito de verdade em lugar nenhum — o Postgres e a máquina passam a ser custo do piloto |
...
O app implementado tem login de verdade: e-mail e senha, sessão, logout, e o
perfil do operador carregado de uma tabela. Existe até um teste exigindo que
nenhuma senha continue no código.

> Desde a **DEC-007** (2026-08-13) a autenticação é própria: a senha vira hash
> `scrypt` numa coluna do banco, e a sessão é um token opaco em cookie
> `httpOnly`, com 12 horas de validade — o suficiente para uma noite inteira,
> que é a resposta direta ao **R11**.
...
| 2026-08-12 | **Os três repositórios viraram um, publicado no GitHub.** `caixa-vivo/` e `imersao-teste-design/` saíram do `.gitignore` da raiz e entraram por merge de subárvore, com os históricos inteiros. `feat/lugar-na-mesa` mergeada na `main` dos dois | Empacotamento, não desenho. Nenhuma linha de produto mudou no caminho. A exclusão existia para o diff do PRD não sumir no meio de build; a proteção passa a ser `git log -- docs/PRD.md`. Custo registrado: commit antigo tem caminho sem prefixo, e o histórico original se lê pelas tags `pre-merge/*` |
| 2026-08-12 | `railway.json` nos dois apps, `allowedHosts` no `vite.config.ts` de cada um, `engines.node`, e o `tsconfig.tsbuildinfo` fora do versionamento | Infraestrutura, não produto. Serve o mesmo bundle que já existia, atrás do domínio da Railway. `allowedHosts` liberado é seguro aqui: o que se serve é bundle público, e a chave `anon` do Supabase é pública por desenho — quem guarda o acesso é o RLS |
| 2026-08-12 | As duas apresentações (`.apresentacao/` e `docs/apresentacao-*.html`) ficaram **fora** do repositório | O repositório é público e apresentação é peça de negócio: quem publica um pitch é o dono, não o build. Uma linha do `.gitignore` reverte |
| 2026-08-13 | **O Supabase saiu inteiro. Backend próprio em TypeScript sobre Postgres puro** — DEC-007. Regra, banco e autenticação mudaram de lugar; `src/regras/` não mudou uma linha | **Arquitetura, não produto.** Nenhuma regra (N1–N19), nenhum critério (A1–A28), nenhum dado da §9 e nenhuma tela da §11 mudou. O que mudou foi **onde** a regra roda: saiu do navegador e foi para o servidor, e isso fecha o **X15**. Os 123 testes provam o comportamento igual. O bundle do cliente caiu de 511 kB para 279 kB, porque o SDK do fornecedor foi junto |
| 2026-08-13 | `Dockerfile` no `caixa-vivo`, e o `railway.json` passa a construir por ele | Empacotamento, não produto. O deploy deixa de depender de o provedor adivinhar como construir: a imagem foi conferida rodando a noite inteira por dentro, contra Postgres zerado. A esteira do GitHub Actions publica a imagem a cada push, e as provas passaram a rodar contra um Postgres criado do zero pelas migrations — resposta permanente ao X12. ⚠️ **O deploy em si não aconteceu:** as duas contas Railway estão travadas em cobrança — `junior-whiteerp` com trial expirado, `andersonszczepanski90` com *"workspace has been restricted, attach a payment method"*. Falta o cartão, não código |
| 2026-08-13 | `public/_redirects` nos dois apps, com `/* /index.html 200` | Hospedagem, não produto. Sem ele, recarregar a página numa rota interna dá 404 no CDN — o servidor procura um arquivo que não existe, porque quem resolve a rota é o react-router dentro do navegador. Nenhuma rota mudou; o arquivo só ensina o CDN a entregar o mesmo `index.html` que o `vite preview` já entregava sozinho |

### Achados de reprodutibilidade

Nenhum é regra de produto. Todos são a mesma doença do **X3**: o sistema
funciona na máquina de hoje por causa de estado que o repositório não carrega.
Quem clonar amanhã não recebe o que está funcionando aqui.

| # | Achado | Onde | Situação |
|---|---|---|---|
| **X12** | **O banco não é reprodutível a partir das migrations.** Não existia nenhum `grant` nas migrations. Os privilégios que o banco local tinha vieram de comando aplicado à mão. | `caixa-vivo/banco/migrations/` | ✅ **fechado** em 2026-08-13 pela DEC-007. A migration `0001` cria o papel `caixa_app`, concede o que ele precisa e o concede a quem roda a migration. **Provado em ambiente limpo:** um Postgres 17 vazio em Docker, migrado do zero, com os 108 testes passando |
| **X15** | **A regra do caixa era sugestão.** O `reducer` rodava no navegador e o navegador escrevia direto no banco com a chave `anon`, que vinha dentro do bundle. O RLS confere *quem* escreve, nunca *o quê*: dava para inserir em `movimentacao` sem passar pelo limite da **N6**, sem o teto de contingências da **N16** e sem a exigência de aceite da **N2**. Achado durante a DEC-007, não por auditoria | `caixa-vivo/src/dados/` | ✅ **fechado** em 2026-08-13. A regra passou para o servidor: o navegador manda a ação e recebe o estado, e não tem caminho para escrever no banco |
| **X13** | **O Design OS não versiona o produto que desenha.** 137 arquivos fora do git: `product/`, `product-plan/`, `src/sections/`, `src/shell/`, `src/simulacao/`. Nada num `.gitignore` os exclui — nunca foram adicionados. É por isso que a auditoria não conseguiu datar a entrada da tela "Ao vivo": o arquivo nunca teve histórico | `imersao-teste-design/` | ✅ **fechado** em 2026-08-12. Conferido no monorepo: `product/` 34 arquivos, `product-plan/` 107, `src/sections/` 43, `src/shell/` 13, `src/simulacao/` 16 — rastreados e publicados. A tela "Ao vivo" tem histórico a partir daqui; o passado dela continua sem data, e isso não tem conserto |
| **X14** | **Os dois `.git` antigos continuam no disco depois do monorepo.** `caixa-vivo/.git` e `imersao-teste-design/.git` seguem funcionando como repositórios próprios. Commit feito lá dentro **não aparece** no monorepo nem no GitHub, e nada avisa | raiz do projeto | 🟠 **aberto**. Decisão do dono: apagar os dois `.git` — o histórico já está no monorepo e as tags `pre-merge/*` marcam a origem — ou mantê-los e commitar sempre a partir da raiz |
