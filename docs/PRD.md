...
| 2026-08-13 | `Dockerfile` no `caixa-vivo`, e o `railway.json` passa a construir por ele | Empacotamento, não produto. O deploy deixa de depender de o provedor adivinhar como construir: a imagem foi conferida rodando a noite inteira por dentro, contra Postgres zerado. ⚠️ **O deploy em si não aconteceu:** as duas contas Railway estão travadas em cobrança — `junior-whiteerp` com trial expirado, `andersonszczepanski90` com *"workspace has been restricted, attach a payment method"*. Falta o cartão, não código |
